...
class RegistrationFeeController extends Controller
{
    public function view()
    {
        ...
        return view('backend.students.regi_fee.view', $data);
    }
    ...
    public function paySlip(Request $request)
    {
        ...
        $pdf = Pdf::loadView('backend.students.regi_fee.fee_payslip', $allStudent);
    ...
    public function getStudentsFee(Request $request)
    {
        ...
        foreach ($allStudent as $key => $v) {
            $registrationfee = CategoryFeeAmount::where('fee_category_id', '2')->where('class_id', $v->class_id)->first();
            $color = 'success';
            ...
            $html[$key]['tdsource'] .= ' <a class="btn btn-sm btn-' . $color . '" title="Payslip" target="_blank" href="' . route('paySlip') . '?class_id=' . $v->class_id . '&student_id=' . $v->student_id . '">Fee Slip</a>';
            $html[$key]['tdsource'] .= '</td>';
        }
